...
class AdminController {
    ...
    static async getContributions(req, res) {
        try {
            ...
            logger_1.logger.error("Get admin contributions error:", error);
            res.status(500).json({ success: false, error: "Internal server error" });
        }
    }
    /**
    ...
    static async getWithdrawals(req, res) {
        try {
            ...
            const withdrawals = await prisma_1.default.withdrawal.findMany({
                where,
                orderBy: { createdAt: "desc" },
                include: {
                    plan: {
            ...
            logger_1.logger.error("Get admin withdrawals error:", error);
            res.status(500).json({ success: false, error: "Internal server error" });
        }
    }
    /**
    ...
    static async getEarnings(req, res) {
        try {
            ...
            logger_1.logger.error("Get admin earnings error:", error);
            res.status(500).json({ success: false, error: "Internal server error" });
        }
    }
    /**
    ...
    static async getDashboard(req, res) {
        try {
            ...
            const [users, plans, contributions] = await Promise.all([
                prisma_1.default.user.findMany({ select: { id: true, name: true, email: true } }),
                prisma_1.default.savingsPlan.findMany({ include: { user: true } }),
                prisma_1.default.contribution.findMany({ include: { plan: { include: { user: true } } } })
            ]);
            ...
            const userDefaulters = new Map();
            // Top Contributors (Users grouped by APPROVED total amount)
            const userContributors = new Map();
    ...
    static async getUserById(req, res) {
        try {
            ...
            logger_1.logger.error("Get admin user by ID error:", error);
            res.status(500).json({ success: false, error: "Internal server error" });
        }
    }
    /**
